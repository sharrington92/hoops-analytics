...
To create the players dimension table, we first must get all the players from the team rosters.
...
# For each team, get roster and insert to db
tbl(duck.con, "teams") %>% 
  filter(!is.na(conf)) %>% 
  ...
  lapply(\(t){
    print(t)
    get_team_roster(team.id = t) %>% 
      mutate(team_id = t) %>% 
      dbAppendTable(duck.con, "temp_roster_raw", .)
  })
...
tbl(duck.con, "temp_roster_raw") %>% 
  select(Player, CleanName, Name) %>% 
  mutate(
    player_name_last = grep(
      "(?<=[:space:]).*", CleanName
    )
  )
...
d <- get_team_roster(team.id = "450680")
